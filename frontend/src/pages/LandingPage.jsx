...
const LandingPage = ({ onGetStarted, onPlayGame }) => {
    return (
        <div className="w-full min-h-full bg-[#111318] text-[#e2e2e9] font-sans relative overflow-x-hidden pb-24 select-none">
            ...
            {/* Hero Section */}
            <main className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 pt-14 pb-16 flex flex-col lg:flex-row items-center justify-between gap-12">
                <div className="flex-1 space-y-6 text-left">
                    <div className="flex items-center gap-3">
                        <img 
                            src={neuronLogoIcon} 
                            alt="Neuron" 
                            className="w-12 h-12 object-contain drop-shadow-[0_0_15px_rgba(245,158,11,0.5)] hover:scale-105 transition-transform" 
                        />
                        <span className="font-serif font-black tracking-widest text-2xl text-white">NEURON</span>
                    </div>

                    <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-500/15 border border-amber-500/30 text-amber-300 font-mono text-xs font-semibold shadow-[0_0_15px_rgba(245,158,11,0.15)]">
                        <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse"></span>
                        <span>AI MEMORY COMPANION &amp; COGNITIVE SANCTUARY</span>
                    </div>

                    <h1 className="font-serif font-bold text-4xl sm:text-6xl lg:text-7xl leading-[1.1] tracking-tight text-white">
                        Your Caring <br />
                    ...
                    <div className="flex flex-wrap items-center gap-4 pt-2">
                        <button
                            ...
                            className="relative group bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-sans text-sm font-bold tracking-wide px-8 py-3.5 rounded-xl shadow-[0_0_25px_rgba(245,158,11,0.25)] flex items-center gap-3 transition-all duration-300 transform hover:-translate-y-0.5 cursor-pointer"
                        >
                        ...
                        <button
                            onClick={onPlayGame}
                            className="relative group bg-[#181a20] hover:bg-[#22252c] text-amber-300 font-sans text-sm font-semibold tracking-wide px-7 py-3.5 rounded-xl border border-amber-500/30 hover:border-amber-400/60 shadow-sm flex items-center gap-2.5 transition-all duration-300 transform hover:-translate-y-0.5 cursor-pointer"
                        >
                            <Gamepad2 size={18} className="text-amber-400" />
                            <span>MEMORY GYM</span>
                        </button>

                        <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl bg-[#181a20] border border-white/[0.06] font-mono text-xs text-slate-400">
                            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                            <span>STATUS: READY &amp; SERENE</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 w-full max-w-md lg:max-w-lg flex justify-center">
                    <div className="w-full bg-[#181a20] border border-amber-500/25 rounded-3xl p-6 shadow-[0_15px_50px_rgba(0,0,0,0.7)] backdrop-blur-xl relative">
                        {/* Terminal Header */}
                        <div className="flex items-center justify-between pb-4 border-b border-white/[0.08] mb-4 text-xs font-mono">
                            <div className="flex items-center gap-2">
                                <span className="w-2.5 h-2.5 rounded-full bg-amber-400"></span>
                                <span className="text-slate-300 font-serif font-semibold text-sm">Neuron Companion</span>
                            </div>
                            <span className="text-amber-400 font-mono text-[11px]">CALM PRESENCE</span>
                        </div>

                        {/* Live Assistant Visual */}
                        <div className="flex items-center gap-4 bg-[#111318] p-3.5 rounded-2xl border border-white/[0.06] mb-4">
                            <img src="/assets/speaking.gif" alt="Avatar" className="w-12 h-12 rounded-xl object-cover border border-amber-500/40 shadow-[0_0_12px_rgba(245,158,11,0.2)]" />
                            <div>
                                <div className="font-serif font-bold text-sm text-white">Caring Memory Companion</div>
                                <div className="font-sans text-xs text-amber-400 flex items-center gap-1.5 mt-0.5">
                                    <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-ping"></span>
                                    <span>Gentle Voice &amp; Vision Ready</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3 font-sans text-xs">
                            <div className="p-3.5 bg-[#1f222a] border border-white/[0.06] text-slate-200 rounded-2xl rounded-tl-sm leading-relaxed">
                            ...
                            <div className="p-3.5 bg-amber-500/15 border border-amber-500/30 text-amber-100 rounded-2xl rounded-tr-sm ml-auto max-w-[85%] text-right leading-relaxed">
            ...
            <div className="max-w-7xl mx-auto px-6 md:px-12 my-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-[#181a20] border border-white/[0.08] rounded-2xl shadow-sm font-sans">
                    <div className="text-center">
                        <div className="text-amber-300 font-serif font-bold text-2xl md:text-3xl">Instant</div>
                        <div className="text-slate-400 text-xs mt-1">Loved Ones Recall</div>
                    </div>
                    <div className="text-center">
                        <div className="text-amber-300 font-serif font-bold text-2xl md:text-3xl">Gentle</div>
                        <div className="text-slate-400 text-xs mt-1">Item Locating</div>
                    </div>
                    <div className="text-center">
                        <div className="text-amber-300 font-serif font-bold text-2xl md:text-3xl">432 Hz</div>
                        <div className="text-slate-400 text-xs mt-1">Calming Soundscapes</div>
                    </div>
                    <div className="text-center">
                        <div className="text-amber-300 font-serif font-bold text-2xl md:text-3xl">100%</div>
                        <div className="text-slate-400 text-xs mt-1">Private &amp; Secure</div>
                    </div>
                </div>
            </div>
